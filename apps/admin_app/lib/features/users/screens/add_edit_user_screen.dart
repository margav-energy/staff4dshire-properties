...
class _AddEditUserScreenState extends State<AddEditUserScreen> {
  ...
  void initState() {
    ...
    // If editing and user is superadmin, allow it. Otherwise, default to staff
    // But if current user is not superadmin, prevent selecting superadmin
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final currentUser = authProvider.currentUser;
    final isCurrentUserSuperadmin = currentUser?.isSuperadmin == true || 
                                    currentUser?.role == UserRole.superadmin;
    
    if (widget.user?.role == UserRole.superadmin && isCurrentUserSuperadmin) {
  ...
  Future<void> _saveUser() async {
    ...
    if (_selectedRole == UserRole.superadmin && !isCurrentUserSuperadmin) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Builder(
                        builder: (context) {
                          // Get current user's role to determine available roles
                          final authProvider = Provider.of<AuthProvider>(context, listen: false);
                          final currentUser = authProvider.currentUser;
                          final isCurrentUserSuperadmin = currentUser?.isSuperadmin == true || 
                                                          currentUser?.role == UserRole.superadmin;
                          
                          // Filter available roles - admins cannot assign superadmin role
                          final availableRoles = isCurrentUserSuperadmin 
                              ? UserRole.values 
