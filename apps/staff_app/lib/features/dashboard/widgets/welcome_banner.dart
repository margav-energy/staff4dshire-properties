...
class _WelcomeBannerState extends State<WelcomeBanner> {
  ...
  Widget build(BuildContext context) {
    ...
    // Use Consumer2 to listen to UserProvider and CompanyProvider changes
    return Consumer2<UserProvider, CompanyProvider>(
      builder: (context, userProvider, companyProvider, child) {
        ...
        if (user.companyId != null && user.companyId!.isNotEmpty) {
          final company = companyProvider.getCompanyById(user.companyId!);
          companyName = company?.name;
          
          // If company not loaded yet, try to load it
          // This will trigger a rebuild via Consumer2 when companies are loaded
          if (companyName == null && companyProvider.companies.isEmpty) {
        ...
        return _buildBannerContent(context, theme, user, photoUrl, companyName);
      },
    );
  }

  Widget _buildBannerContent(BuildContext context, ThemeData theme, User user, String? photoUrl, String? companyName) {
