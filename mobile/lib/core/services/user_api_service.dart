...
class UserApiService {
  ...
  static Future<List<UserModel>> getUsers() async {
    try {
      ...
      if (response is List) {
        return response.map((json) => _fromApiJson(json as Map<String, dynamic>)).toList();
      } else if (response is Map<String, dynamic> && response.containsKey('data')) {
        final List<dynamic> users = response['data'] as List;
        return users.map((json) => _fromApiJson(json as Map<String, dynamic>)).toList();
      }
      
      return [];
    } catch (e) {
      print('UserApiService.getUsers error: $e');
      rethrow;
    }
  }
