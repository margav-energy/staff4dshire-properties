-- Migration: Add Chat Tables (v2 - uses gen_random_uuid())
...
-- Trigger for Updated At on conversations (only if function exists)
DO $$
BEGIN
...
-- Trigger for Updated At on messages (only if function exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
